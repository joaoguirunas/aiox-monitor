        ...
        <div className="space-y-6">
          ...
          {processing.length > 0 && (
            <TerminalSection label="Processando" count={processing.length} dot="bg-accent-blue">
              {processing.map(t => (
                <TrackedTerminalCard key={t.id} pid={t.pid} status={t.status} projectName={t.project_name} windowTitle={t.window_title} agentName={t.agent_name} agentDisplayName={t.agent_display_name} currentTool={t.current_tool} currentInput={t.current_input} currentToolDetail={t.current_tool_detail} waitingPermission={t.waiting_permission} firstSeen={t.first_seen_at} lastActive={t.last_active} />
              ))}
            </TerminalSection>
          )}
