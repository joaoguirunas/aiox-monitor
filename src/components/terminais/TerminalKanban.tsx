...
function InactiveChip({ terminal }: { terminal: TerminalWithMeta }) {
  ...
  return (
    <span className="inline-flex items-center gap-2 px-2.5 py-1.5 rounded-md bg-surface-2/40 border border-border/30 hover:bg-surface-2/60 transition-colors">
      ...
      <span className="text-[11px] text-text-secondary truncate max-w-[140px]">{title}</span>
...
export function TerminalKanban({ terminals }: TerminalKanbanProps) {
  ...
  return (
    <div className="grid grid-cols-[1fr_1fr_1fr] divide-x divide-border/20">
      {/* Disponíveis */}
      <StatusColumn label="Disponíveis" dot="bg-zinc-500" count={inactive.length}>
        {inactive.length > 0 && (
      ...
      {/* Trabalhando */}
      <StatusColumn label="Trabalhando" dot="bg-emerald-400" dotAnimate count={processing.length}>
        {processing.length > 0 && (
          <div className="flex flex-col gap-1.5">
...
function StatusColumn({ label, dot, dotAnimate, count, children }: {
  label: string;
  dot: string;
  dotAnimate?: boolean;
  count: number;
  ...
  return (
    <div className="px-4 py-3 min-h-[120px]">
      ...
      {count === 0 ? (
        <p className="text-[10px] text-text-muted/40 py-2">Nenhum</p>
      ) : children}
    </div>
  );
}
