...
function testSchema() {
  ...
  try {
    ...
    // Check columns
    const columns = db.prepare(`PRAGMA table_info(command_room_terminals)`).all();
    ...
    // Check index
    const indexes = db.prepare(`PRAGMA index_list(command_room_terminals)`).all();
...
function testCRUD() {
  ...
  try {
    ...
    const updated = db.prepare(`SELECT cols, rows FROM command_room_terminals WHERE id = ?`).get(testId);
    if (updated.cols !== 300 || updated.rows !== 60) {
    ...
    const closed = db.prepare(`SELECT pty_status FROM command_room_terminals WHERE id = ?`).get(testId);
    if (closed.pty_status !== 'closed') {
...
function testCrashedLogic() {
  ...
  try {
    ...
    const t1 = db.prepare(`SELECT pty_status FROM command_room_terminals WHERE id = ?`).get(terminal1);
    const t2 = db.prepare(`SELECT pty_status FROM command_room_terminals WHERE id = ?`).get(terminal2);

    if (t1.pty_status !== 'active') {
      error('Terminal1 should remain active');
      return false;
    }
    if (t2.pty_status !== 'crashed') {
    ...
    error(`Crashed logic test failed: ${err.message}`);
    return false;
  }
}
